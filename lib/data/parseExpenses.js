...
const REQUIRED_COLUMNS = [
  ...
  'USD Equivalent Amount', 'Year', 'Month', 'Merchant Name', 'Type',
  'Frequency',
];
...
function buildColumnMap(headerRow) {
  ...
  for (let i = 0; i < headerRow.length; i++) {
    const cell = typeof headerRow[i] === 'string' ? headerRow[i].trim() : headerRow[i];
...
export function parseExpenses(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const rawFreq = String(row[colMap['Frequency']] || '').trim();
    // 'Frequency' has case-variant ad-hoc values in the sheet
    // ('Ad-hoc' vs 'Ad-Hoc' — 20 and 2 rows respectively). Normalize at
    // parse time to a boolean so downstream consumers can't miss either
    // variant by accident.
    const isAdhock = rawFreq.toLowerCase() === 'ad-hoc';
    results.push({
      ...
      month: MONTH_NAME_TO_NUMBER[monthStr] || (Number.isFinite(Number(monthStr)) && Number(monthStr) >= 1 && Number(monthStr) <= 12 ? Number(monthStr) : 0),
