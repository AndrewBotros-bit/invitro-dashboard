import { batchGetSheetValues, getExpenseSheetValues, getHeadcountSheetValues, getRevenueDetailsSheetValues, batchGetFundTimelineSheetValues } from '@/lib/googleSheets';
...
import { parseRevenueDetails } from '@/lib/data/parseRevenueDetails';
import { parseIRRValuation } from '@/lib/data/parseIRR';
import { parseFundTimeline } from '@/lib/data/parseFundTimeline';
...
export async function fetchAllData() {
  ...
  // External AllRx + AllCare tabs are fetched in the same batchGet so the
  // build only pays one round trip. They share the main P&L/Cashflow
  // structure (company name in col B, metric in col C, monthly values from
  // col E), so we reuse parsePnL/parseCashflow on them and rename each
  // block from the internal name → "{internal} External" before appending.
  // See Dashboard.jsx applyExternalCompanyView for per-user view rules.
  const [mainRanges, expenseRows, headcountRows, allCareRevRows, allRxRevRows, fundTimelineRanges] = await Promise.all([
    batchGetSheetValues({
      spreadsheetId: mainSheetId,
      // IRR range:
      //   - rows: 250 (added per-LP-per-year investment rows doubled the LP
      //     section's row count; the old 180-row cap silently truncated
      //     later vehicles' LPs).
      //   - cols: R (2028 forecast). Year cols are D=2021, F=2022, …,
      //     P=2027, R=2028. We fetch through R even when YEAR_COLS only
      //     declares through 2027 — over-fetching costs nothing and
      //     future-proofs against the next year-add request.
      ranges: [
        "'P&L'!A1:ZZ",
        "'Cashflow'!A1:ZZ",
        "'IRR & Valuation'!A1:R250",
        "'AllRx P&L External'!A1:ZZ",
        "'AllRx Cashflow External'!A1:ZZ",
        "'AllCare P&L External'!A1:ZZ",
        "'AllCare Cashflow External'!A1:ZZ",
      ],
    }),
    getExpenseSheetValues("'Row Data (2026)'!A1:S"),
    getHeadcountSheetValues("'Employees List'!A1:AZ"),
    getRevenueDetailsSheetValues("AllCare!A1:R"),
    getRevenueDetailsSheetValues("AllRx!A1:R"),
    // Fund Timeline sheet (separate spreadsheet). Optional — returns [] if
    // INVITRO_FUND_TIMELINE_SHEET_ID unset, in which case parseFundTimeline
    // returns null and IRRValuation falls back to annual-XIRR from the IRR
    // sheet. 'Cash Flow Timeline' = per-LP monthly capital calls;
    // 'Ownerhsip %' = annual mgmt-fee-adjusted ownership snapshot (SIC —
    // typo in sheet tab name is intentional to match the source).
    batchGetFundTimelineSheetValues([
      "'Cash Flow Timeline'!A1:BZ20",
      "'Ownerhsip %'!A1:K20",
    ]),
  ]);
  ...
  const irrRows = mainRanges[2]?.values ?? [];
  const pnlExtRows = mainRanges[3]?.values ?? [];
  const cashflowExtRows = mainRanges[4]?.values ?? [];
  const allCarePnlExtRows = mainRanges[5]?.values ?? [];
  const allCareCashflowExtRows = mainRanges[6]?.values ?? [];
  ...
  // External AllRx + AllCare (legal-public-target views). Same parsers,
  // then rename each block from its internal name → "{internal} External"
  // and append. Per-user view rules in Dashboard.jsx decide whether each
  // user sees the internal, external, or both as separate sidebar entries.
  const pnlExt = parsePnL(pnlExtRows)
    .filter(c => c.name === 'AllRx')
    .map(c => ({ ...c, name: 'AllRx External' }));
  const cashflowExt = parseCashflow(cashflowExtRows)
    .filter(c => c.name === 'AllRx')
    .map(c => ({ ...c, name: 'AllRx External' }));
  if (pnlExt.length === 0) console.warn('[DATA] AllRx P&L External tab parsed empty (no "AllRx" block found)');
  if (cashflowExt.length === 0) console.warn('[DATA] AllRx Cashflow External tab parsed empty (no "AllRx" block found)');
  pnl.push(...pnlExt);
  cashflow.push(...cashflowExt);

  const allCarePnlExt = parsePnL(allCarePnlExtRows)
    .filter(c => c.name === 'AllCare')
    .map(c => ({ ...c, name: 'AllCare External' }));
  const allCareCashflowExt = parseCashflow(allCareCashflowExtRows)
    .filter(c => c.name === 'AllCare')
    .map(c => ({ ...c, name: 'AllCare External' }));
  if (allCarePnlExt.length === 0) console.warn('[DATA] AllCare P&L External tab parsed empty (no "AllCare" block found)');
  if (allCareCashflowExt.length === 0) console.warn('[DATA] AllCare Cashflow External tab parsed empty (no "AllCare" block found)');
  pnl.push(...allCarePnlExt);
  cashflow.push(...allCareCashflowExt);

  const expenses = parseExpenses(expenseRows);
  const headcount = parseHeadcount(headcountRows);
  const revenueDetails = parseRevenueDetails(allCareRevRows, allRxRevRows);
  const irrValuation = parseIRRValuation(irrRows);

  // Fund Timeline — attach onto irrValuation.fundTimeline for the InVitro
  // Fund vehicle. Returns null when the Timeline sheet was unreachable
  // (env var unset, share missing, or empty ranges); IRRValuation.jsx
  // falls back to annual XIRR from the IRR sheet in that case.
  const timelineRows = fundTimelineRanges[0]?.values ?? [];
  const ownershipRows = fundTimelineRanges[1]?.values ?? [];
  const fundTimeline = parseFundTimeline(timelineRows, ownershipRows);
  if (fundTimeline) {
    if (!irrValuation.fundTimelines) irrValuation.fundTimelines = {};
    irrValuation.fundTimelines['InVitro Fund'] = fundTimeline;
    console.log(`[FUND_TIMELINE] Parsed ${Object.keys(fundTimeline.perLp).length} LP series`);
  } else {
    console.warn('[FUND_TIMELINE] Not available — IRR will fall back to annual XIRR');
  }
  ...
  const data = { pnl, cashflow, expenses, headcount, revenueDetails, irrValuation, companies, cashRunwayRow, lastActualMonth, fetchedAt: new Date().toISOString() };
  // TEMP: log the shape of what gets serialized to the client so we can
  // see if fundTimelines survives the RSC → client boundary in prod.
  const _ft = data.irrValuation?.fundTimelines?.['InVitro Fund'];
  const _bot = _ft?.perLp?.['Fr. Botros Samy'];
  console.log('[FUND_TIMELINE][SHAPE]',
    'attached=', !!_ft,
    'lps=', Object.keys(_ft?.perLp ?? {}).length,
    'botFlows=', _bot?.flows?.length ?? 0,
    'botOwn=', Object.keys(_bot?.ownershipByYear ?? {}).length);
