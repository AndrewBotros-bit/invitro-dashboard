...
export async function fetchAllData() {
  ...
  const [mainRanges, expenseRows, headcountRows, allCareRevRows, allRxRevRows] = await Promise.all([
    batchGetSheetValues({
      ...
      // IRR range extended to row 250 — when Andrew added per-LP-per-year
      // investment rows, each LP grew from 1 row to 2 rows in the sheet.
      // The old 180-row cap was sized to the previous layout and was silently
      // truncating later LPs (only Fr. Botros Samy of InVitro Fund came
      // through; all of InVitro Ventures' LPs were cut off).
      ranges: ["'P&L'!A1:ZZ", "'Cashflow'!A1:ZZ", "'IRR & Valuation'!A1:O250"],
    }),
