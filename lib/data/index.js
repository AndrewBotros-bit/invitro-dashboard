...
export async function fetchAllData() {
  ...
  // External AllRx tabs are fetched in the same batchGet so the build only
  // pays one round trip. They share the main P&L/Cashflow structure (company
  // name in col B, metric in col C, monthly values from col E), so we reuse
  // parsePnL/parseCashflow on them and just rename the "AllRx" block to
  // "AllRx External" before appending. See Dashboard.jsx for the per-user
  // view-substitution rules.
  ...
  // External AllRx (legal-public-target view). Same parsers, then rename the
  // block from "AllRx" → "AllRx External" and append. Per-user view rules in
  // Dashboard.jsx decide which one ("AllRx" vs "AllRx External") each user
  // sees in their portfolio sidebar.
