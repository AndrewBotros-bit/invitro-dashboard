...
/**
 * Where a company block stops when nothing follows it in the company
 * list: the first "Total" roll-up or vehicle block after it. Falls back
 * to the end of the sheet only if neither is found.
 */
function companyBlockEnd(rows, start) {
  for (let i = start + 1; i < rows.length; i++) {
    const b = colB(rows[i]);
    if (!b) continue;
    if (/^total\b/i.test(b)) return i;
    if (VEHICLE_NAMES.includes(b) || SKIP_VEHICLES.has(b)) return i;
  }
  return rows.length;
}
...
function parseCompanyBlock(rows, start, end, periods) {
  ...
  for (let i = start; i < end && i < rows.length; i++) {
    ...
    // Prefix-matched: the sheet renamed this row from "ARR as of
    // December" to "ARR (last month x 12), $". Exact matching missed the
    // new label, so ARR stopped being read AND the row fell through to
    // the direct-shareholder catch-all below — see that comment.
    else if (/^ARR\b(?!\s*Growth)/i.test(c)) financials.arr = vals;
    else if (c === 'Annual Gross Margin, %' || c.startsWith('Gross Margin')) financials.grossMargin = vals;
    else if (c.startsWith('Annual Revenues Growth Rate') || c.startsWith('Revenue Growth')
             || /^ARR\s*Growth/i.test(c)) financials.revGrowth = vals;
    ...
    // A row is an INVESTMENT row only if it says so. Anything of the
    // form "<Name> Investment, $" is either one of the known vehicles or
    // a named investor holding directly (e.g. "New Investor").
    //
    // This used to be a catch-all on any label ending in ", $", which
    // swept up "ARR (last month x 12), $" as though ARR were a
    // shareholder and added it to Total Invested. AllCare + Curenta read
    // $18.28m against $5.14m of real capital; InVitro Studio read
    // $96.92m against $3.63m. ARR is a stock figure repeated every
    // period, so summing the series inflated it further.
    else if (/\binvestment,\s*\$\s*$/i.test(c)) {
      const v = matchVehicle(c);
      if (v) {
        investments[v] = vals;
      } else {
        const name = c.replace(/\s*investment,\s*\$\s*$/i, '').trim();
        if (name) upsertDirect(name, 'investment', vals);
      }
    }
    else if (/\bowner(?:s)?hip,?\s*%/i.test(c) || /\bownerhsip,?\s*%/i.test(c)) {
      const v = matchVehicle(c);
      if (v) {
        ownership[v] = vals;
      } else {
        const name = c.replace(/\s*owner(?:s)?hip,?\s*%.*$/i, '')
                      .replace(/\s*ownerhsip,?\s*%.*$/i, '').trim();
...
export function parseIRRValuation(rows) {
  ...
  for (let i = 0; i < companyBlocks.length; i++) {
    ...
    // The LAST company block used to fall back to a hardcoded row 100,
    // which ran straight through the "Total" roll-up beneath it. InVitro
    // Studio has no ARR row of its own, so it picked up Total's and
    // reported $16.15m — the sum of AllRx and AllCare + Curenta — as if
    // it were Studio's. Bound the block at whatever actually follows it.
    const end = companyBlocks[i + 1]?.startRow ?? companyBlockEnd(rows, block.startRow);
