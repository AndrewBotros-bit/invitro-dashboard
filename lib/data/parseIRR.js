...
function parseVehicleBlock(rows, start) {
  ...
  // Read forward until the NEXT vehicle anchor. A vehicle anchor is
  // detected STRUCTURALLY (col B non-empty AND col C == "Shareholders
  // ownerhsip, $") — not by name allowlist. The earlier whitelist-only
  // check (VEHICLE_NAMES + SKIP_VEHICLES) was brittle: when the CFO
  // added a new "New Investment Vehicle" block at the bottom of the
  // rollup section, its name wasn't in either set, the loop didn't stop,
  // and the new vehicle's zeros / formula errors overwrote the PRIOR
  // vehicle's ownership/investment/IRR/MOIC values. Structural detection
  // catches any new vehicle block regardless of name, while still
  // ignoring continuation rows (which have col B empty).
