...
function parseCompanyBlock(rows, start, end) {
  ...
  for (let i = start; i < end && i < rows.length; i++) {
    ...
    else if (c.endsWith('Investment, $')) {
      const v = VEHICLE_NAMES.find(name => c.startsWith(name));
      if (v) investments[v] = vals;
    }
    else if (c.endsWith('Ownership, %') || c.endsWith('ownerhsip, %')) {
      const v = VEHICLE_NAMES.find(name => c.startsWith(name));
...
function parseVehicleBlock(rows, start) {
  ...
  // Read up to 8 rows forward (the block is ~7 rows: ownership $, ownership %, blank, hold, inv, irr, moic).
  for (let i = start; i < start + 9 && i < rows.length; i++) {
    const r = rows[i];
    if (!r) continue;
    const c = colC(r);
    const vals = rowToYearValues(r);
