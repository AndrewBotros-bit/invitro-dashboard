 ...
 *
 * Optional env var — if unset, returns []. Downstream parsers treat no-data
 * as "fall back to annual XIRR from the IRR sheet".
 */
export async function getFundTimelineSheetValues(range) {
  const spreadsheetId = process.env['INVITRO_FUND_TIMELINE_SHEET_ID'];
  if (!spreadsheetId) return [];
  try {
    const sheets = createSheetsClient();
