 ...
 *  'AllRx External' is a per-user alternate view of AllRx and must never roll
 *  into consolidated aggregates — Andrew's rule: consolidated always uses the
 *  internal "AllRx" entry. Dashboard.jsx renames external→AllRx for users
 *  who only have external access, so this filter only blocks the case where
 *  both entries coexist (admin view). */
export const EXCLUDE_ALWAYS = ['InVitro Holding', 'AllRX Holding', 'Confider', 'Yalent', 'Curenta', 'ALL Holdings', 'Add hocks', 'AllRx External', 'AllCare External'];
...
/**
 * Build quarterly aggregated series from P&L data.
 * Returns one entry per (year, quarter) bucket with totals per company.
 * Label format: "Q{quarter} {yearTwoDigits}" e.g. "Q1 25".
 *
 * Range filtering supports BOTH year-only and quarter-granularity:
 *   - Old shape: pass numbers for fromYear/toYear (filters by year only).
 *   - New shape: pass objects { year, month } — the function computes the
 *     containing quarter for each endpoint and filters to that range.
 *   Detection is by typeof — number → year-only mode; object → quarter mode.
 *
 * Use the object form when the caller wants the chart to respect a
 * quarter-granularity range (e.g., Q2 26 → Q4 26 should NOT include Q1 26).
 *
 * @param {CompanyPnL[]} pnlData
 * @param {string} metricName
 * @param {string[]} excludeCompanies
 * @param {number|{year:number,month:number}|null} from
 * @param {number|{year:number,month:number}|null} to
 * @returns {Array<{month: string, _year: number, _quarter: number, _month: number, [companyName: string]: number}>}
 */
export function buildQuarterlySeries(pnlData, metricName, excludeCompanies = [], from = null, to = null) {
  const excludeSet = new Set(excludeCompanies);
  const companies = pnlData.filter(c => !excludeSet.has(c.name));
  if (companies.length === 0) return [];

  // Normalize range endpoints to (year, quarter) tuples. Accepts either
  // a bare year number (legacy year-only filter) or an object with
  // {year, month} (full quarter-granularity filter).
  const toBound = (v) => {
    if (v === null || v === undefined) return null;
    if (typeof v === 'number') return { year: v, quarter: null }; // year-only — no quarter bound
    return { year: v.year, quarter: Math.ceil(v.month / 3) };
  };
  const fromBound = toBound(from);
  const toEndBound = toBound(to);
  // Compare a bucket against a bound. quarter=null means "no quarter
  // bound on this endpoint" (legacy behavior).
  const afterOrEqualFrom = (b) => {
    if (!fromBound) return true;
    if (b.year !== fromBound.year) return b.year > fromBound.year;
    return fromBound.quarter === null ? true : b.quarter >= fromBound.quarter;
  };
  const beforeOrEqualTo = (b) => {
    if (!toEndBound) return true;
    if (b.year !== toEndBound.year) return b.year < toEndBound.year;
    return toEndBound.quarter === null ? true : b.quarter <= toEndBound.quarter;
  };

  // Collect all year-quarter buckets
  const buckets = new Set();
  for (const company of companies) {
    const vals = company.metrics[metricName];
    if (!vals) continue;
    for (const mv of vals) {
      const q = Math.ceil(mv.month / 3);
      buckets.add(`${mv.year}-${q}`);
    }
  }
  const sortedBuckets = [...buckets]
    .map(b => { const [y, q] = b.split('-').map(Number); return { year: y, quarter: q }; })
    .filter(b => afterOrEqualFrom(b) && beforeOrEqualTo(b))
    .sort((a, b) => a.year !== b.year ? a.year - b.year : a.quarter - b.quarter);

  return sortedBuckets.map(({ year, quarter }) => {
    const fromMonth = (quarter - 1) * 3 + 1;
    const toMonth = quarter * 3;
    const entry = { month: `Q${quarter} ${String(year).slice(-2)}`, _year: year, _quarter: quarter, _month: toMonth };
    for (const company of companies) {
      const vals = (company.metrics[metricName] ?? []).filter(v => v.year === year && v.month >= fromMonth && v.month <= toMonth);
      entry[company.name] = vals.reduce((s, v) => s + (v.value ?? 0), 0);
    }
    return entry;
  });
